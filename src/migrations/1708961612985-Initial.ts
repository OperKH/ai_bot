...
export class Initial1708961612985 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Originally pgvecto.rs (`vectors`) with its own index. The VectorChord
        // images the later migrations need do not ship it, so a fresh database
        // starts on pgvector's `vector` type, and MigrateToVectorChord builds the
        // vector index.
